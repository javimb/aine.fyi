...
Declare Spanish (Spain) as the page language via the HTML `lang` attribute and Open Graph locale metadata. Locale configuration is sourced from next-intl (`src/i18n/config.ts`), and page metadata strings (title, description) are sourced from the next-intl message catalog (`messages/es-ES.json`).
