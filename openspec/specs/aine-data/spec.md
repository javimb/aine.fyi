...
The project SHALL define a typed TypeScript data structure representing AINE (NSAID) entries in `data/aines.ts`. Each entry SHALL include: `name` (canonical Spanish name), `aliases` (alternative names including brand names), and `family` (pharmacological family category).

#### Scenario: AINE entry contains required fields

- **WHEN** an AINE entry is defined in the blacklist
- **THEN** it SHALL contain at minimum a `name`, an `aliases` array, and a `family` string
