...
- Update `next.config.ts` to skip ESLint and TypeScript checks during build (CI already handles these)
...
- `next.config.ts` — adds `eslint.ignoreDuringBuilds` and `typescript.ignoreBuildErrors`
